...
//         address impl = address(new Realms());
...
//         IBridge(bridge).enableBridge(true);
//     }
...
//         IBridge(bridge).enableBridge(false);
...
//         IBridge(bridge).enableWhiteList(true);
...
//         IBridge(bridge).enableWhiteList(false);

//         IBridge(bridge).whiteList(erc721C1, true);
...
//         IBridge(bridge).whiteList(erc721C1, false);

//     }

//     function test_isWhiteListEnabled() public {
//         IBridge(bridge).enableWhiteList(false);
//         assert(!IBridge(bridge).isWhiteListEnabled());

//         IBridge(bridge).enableWhiteList(true);
//         assert(IBridge(bridge).isWhiteListEnabled());

//         IBridge(bridge).whiteList(erc721C1, true);
//         assert(IBridge(bridge).isWhiteListed(erc721C1));
...
//         address[] memory whiteListed = IBridge(bridge).getWhiteListedCollections();
//         assertEq(whiteListed.length, 0);

//         IBridge(bridge).whiteList(erc721C1, true);
//         IBridge(bridge).whiteList(erc1155C1, true);
//         whiteListed = IBridge(bridge).getWhiteListedCollections();
//         assertEq(whiteListed.length, 2);

//         IBridge(bridge).whiteList(erc1155C1, false);
//         whiteListed = IBridge(bridge).getWhiteListedCollections();
...
//         IBridge(bridge).addMessageHashForAutoWithdraw(uint256(msgHash));
...
//         address collection = IBridge(bridge).withdrawTokens(reqSerialized);
...
//         address collection1 = IBridge(bridge).withdrawTokens(reqSerialized);
...
//         msgHashes[0] = uint256(msgHash);
//         IStarknetMessagingLocal(snCore).addMessageHashesFromL2(msgHashes);
//         address collection2 = IBridge(bridge).withdrawTokens(reqSerialized);
...
//         IBridge(bridge).enableWhiteList(true);
//         assert(IBridge(bridge).isWhiteListEnabled());
//         assert(IBridge(bridge).isWhiteListed(collection));
//     }

//     function test_depositWithdrawTokens_withMapping() public {
//         // add mapping L1 <-> L2: erc721C1 <-> 0x123
//         IBridge(bridge).setL1L2CollectionMapping(address(erc721C1), Cairo.snaddressWrap(0x123), true);
...
//         IBridge(bridge).enableBridge(true);
//         assert(IBridge(bridge).isEnabled());
...
//         returns (bytes32)
//     {
//         (snaddress realmsL2Address, felt252 realmsL2Selector)
//             = IBridge(bridge).l2Info();

//         // To remove warning. Is there a better way?
//         assertTrue(felt252.unwrap(realmsL2Selector) > 0);

//         bytes32 msgHash = keccak256(
//             abi.encodePacked(
//                 snaddress.unwrap(realmsL2Address),
